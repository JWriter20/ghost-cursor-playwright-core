...
export const createCursor = (
  ...
  const performMicroJitter = async (center: Vector, duration: number = 200): Promise<void> => {
    const startTime = Date.now()
    while (Date.now() - startTime < duration) {
      const jitterPoint = addNoise(center, 3 + Math.random() * 2)
      await tracePath([{ x: jitterPoint.x, y: jitterPoint.y }])
      await delay(20 + Math.random() * 30)
    }
    await tracePath([{ x: center.x, y: center.y }])
  }
    ...
    async click(
      ...
      try {
        ...
        if (selector != null) {
          ...
          if (optionsResolved.microJitter === true) {
            await performMicroJitter({ x: boundingBox.x + offset.x, y: boundingBox.y + offset.y }, 100 + Math.random() * 200)
          }
          ...
          if (optionsResolved.microJitter === true) {
            await performMicroJitter(previous, 100 + Math.random() * 200)
          }
